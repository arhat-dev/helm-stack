...
GO_LDFLAGS="-s -w \
  -X arhat.dev/helm-stack/pkg/version.branch=${GIT_BRANCH} \
  -X arhat.dev/helm-stack/pkg/version.commit=${GIT_COMMIT} \
  -X arhat.dev/helm-stack/pkg/version.tag=${GIT_TAG} \
  -X arhat.dev/helm-stack/pkg/version.arch=${ARCH} \
  -X arhat.dev/helm-stack/pkg/version.goCompilerPlatform=$(go version | cut -d\  -f4)"
