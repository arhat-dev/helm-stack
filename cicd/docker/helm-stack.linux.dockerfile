...
RUN export KUBE_ARCH="$(basename ${ARCH} v7)" &&\
    ...
    curl -SsL "https://get.helm.sh/helm-v2.16.12-linux-${KUBE_ARCH}.tar.gz" | tar -zxf - -C /tmp/helm &&\
    mv "/tmp/helm/linux-${KUBE_ARCH}/helm" /opt/helm/v2/helm &&\
    rm -rf /tmp/helm &&\
    mkdir -p /tmp/helm /opt/helm/v3 &&\
    curl -SsL "https://get.helm.sh/helm-v3.3.4-linux-${KUBE_ARCH}.tar.gz" | tar -zxf - -C /tmp/helm &&\
